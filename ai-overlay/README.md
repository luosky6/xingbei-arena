...
> 以**叠加层(overlay)**形式加载，**绝不修改 `noname_xingbei_clone/` 任何源文件**。
...
3. **自对弈调参**：固定种子大批对局，用 CEM/网格优化 `weights.json`，目标=对原版内置AI胜率最大化。
...
- `valueFn.js` — V 与 victim 估值的可运行骨架（含 TODO）。
- `install.js` — overlay 安装/猴补丁骨架（含 hook 点说明）。
